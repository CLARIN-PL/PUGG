"""
This file is a copy of beir library implementation of BM25Search based on Elasticsearch.
...
import tqdm
from src.es_beir_patch import ElasticSearch
...
class BaseSearch(ABC):
    @abstractmethod
    def search(
        self, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], top_k: int, **kwargs
        ...
        **kwargs
        ...
        for start_idx in tqdm.trange(0, len(queries), self.batch_size, desc="que"):
            ...
            results = self.es.lexical_multisearch(
                texts=queries[start_idx : start_idx + self.batch_size], top_hits=top_k + 1
